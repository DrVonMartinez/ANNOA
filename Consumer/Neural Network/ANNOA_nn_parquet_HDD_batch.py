...
class Ozturk:
    def __init__(self, size: int = 200, dimension: int = 1, hidden_neurons: int = 250, optimizer: str = 'adam', epochs: int = 100, full_classes: bool = False, full_data: bool = False,
        ...
        self.__steps_per_epoch = np.ceil(MONTE_CARLO / self.__batch_size)
    ...
    def batch_generator(self):
        index = np.zeros(len(self.__distribution_names), dtype=np.int32)
        ...
        while True:
            ...
            for i in range(len(unique_files)):
                file = unique_files[i]
                rows = list(range(index[file], index[file] + unique_batch_count[i]))
                if index[file] + unique_batch_count[i] > MONTE_CARLO:
                    rows = list(range(index[file], MONTE_CARLO)) + list(range(0, index[file] + unique_batch_count[i] - MONTE_CARLO))
                instance = self.__load_instance(rows, file)
                index[file] += unique_batch_count[i]
            ...
            if training.shape != (self.__batch_size, len(self.__batch_columns)):
                raise ValueError('Training Shape does not follow format (Batch Size, len(Batch Columns))\n[' + ', '.join(list(testing.columns)) + ']')
            if testing.shape != (self.__batch_size, len(self.__distribution_names)):
    ...
    def __load_instance(self, rows: list, file_index: int):
        # df = pd.read_parquet(self.__distribution_files[file_index]).loc[rows]
        reader = ParquetFile(self.__distribution_files[file_index])
        pyarrow_table: pa.Table = reader.iter_batches(len(rows), row_groups=rows)
        print(self.__distribution_files[file_index])
        # pyarrow_table: pa.Table = reader.read_row_groups(rows, use_pandas_metadata=True)
        return next(pyarrow_table).to_pandas().loc[rows]

    def __load_data(self, train_df, idx):
        df = pd.read_csv(train_df, skiprows=idx*self.__batch_size, nrows=self.__batch_size)
        x = df[self.__batch_columns].to_numpy(dtype=self.__dtype)
        y = df[self.__distribution_names].to_numpy(dtype=self.__dtype)
        return x, y
